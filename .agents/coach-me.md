...
0. If this is iteration 001 and the student has no factory implementation yet, ask what language they want to build it in before starting. There is no prescribed stack, however the language must support a CLI interface. Encourage the user to choose a language they're familiar with so they can review the code. For later iterations, keep using whatever they already chose.
