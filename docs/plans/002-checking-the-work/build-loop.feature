Feature: Build loop
  ...
  The factory builds software from a seed, one task at a time. A pass is
  now a doer and a validator working until the validator is satisfied —
  see `checking-the-work.feature`. Everything below is about the outer
  loop: the plan, the tasks, and when the factory stops.
