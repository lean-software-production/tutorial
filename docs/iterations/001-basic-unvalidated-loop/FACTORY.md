...
job has a seed and a target. The seed is a file, `spec.md`, describing a
game of Tetris for the terminal. The target is the codebase the job
builds: its own git repository, holding only the product. If the target
isn't a git repository yet, it becomes one, with a starting commit.
